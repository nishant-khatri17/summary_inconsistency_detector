...
parser.add_argument("--topk",          type=int,   default=3,
                    help="Number of top doc sentences to average in top-k strategy.")
parser.add_argument("--blend_w",       type=float, default=0.5,
                    help="Weight of ZS component in hybrid blend (1-w for net-ent).")
...
# ==============================
# NEW STRATEGIES — Contradiction-Aware & Robust Aggregation
# ==============================

# --- Strategy 6: Net-Entailment Score ---
def agg_net_entailment(E, Neu, C):
    """
    For each summary sentence, take max(E - C) across doc sentences,
    then average. Directly penalizes contradiction — something ZS
    completely ignores. Simple but addresses a fundamental blind spot.
    """
    net = E - C                                    # (M, N)
    col_max = net.max(dim=0).values                # (N,)
    return col_max.clamp(0.0, 1.0).mean().item()


# --- Strategy 7: Scaled Entailment ---
def agg_scaled_entailment(E, Neu, C):
    """
    Multiplicative contradiction gate: E × (1 - C).
    Kills entailment scores where the model also assigns high
    contradiction probability (confused/uncertain predictions).
    More aggressive than additive net-entailment.
    """
    scaled = E * (1.0 - C)                         # (M, N)
    col_max = scaled.max(dim=0).values             # (N,)
    return col_max.mean().item()


# --- Strategy 8: Top-K Mean ---
def agg_topk_mean(E, Neu, C):
    """
    Instead of hard max (susceptible to spurious spikes), average
    the top-k entailment scores per summary sentence. Requires
    convergent evidence from multiple doc sentences.
    """
    k = min(args.topk, E.shape[0])
    topk_vals = E.topk(k, dim=0).values            # (k, N)
    col_scores = topk_vals.mean(dim=0)              # (N,)
    return col_scores.mean().item()


# --- Strategy 9: Row-Normalized Entailment ---
def agg_row_normalized(E, Neu, C):
    """
    Z-score normalize entailment per doc sentence (row) to suppress
    'hot' sentences that entail everything equally. After normalization,
    max picks the doc sentence most SPECIFICALLY supporting each claim.
    Sigmoid maps z-scores back to [0, 1].
    Falls back to ZS for single-claim summaries.
    """
    M, N = E.shape
    if N <= 1:
        return agg_zs(E, Neu, C)

    row_mean = E.mean(dim=1, keepdim=True)          # (M, 1)
    row_std  = E.std(dim=1, keepdim=True).clamp(min=1e-6)
    E_z      = (E - row_mean) / row_std             # (M, N) z-scores
    col_max  = E_z.max(dim=0).values                # (N,)
    scores   = torch.sigmoid(col_max)               # (N,) → [0, 1]
    return scores.mean().item()


# --- Strategy 10: Hybrid Blend (ZS + Net-Entailment) ---
def agg_hybrid_blend(E, Neu, C):
    """
    Weighted combination of ZS max and net-entailment scores.
    ZS provides strong baseline signal; net-entailment adds
    contradiction awareness. Blend captures complementary info.
    """
    w = args.blend_w
...
# --- Strategy 11: SIFiD + Net-Entailment ---
def agg_sifid_net_ent(E, Neu, C):
    """
    SIFiD document filtering followed by net-entailment scoring.
    Combines noise removal with contradiction awareness.
    """
    mask = sifid_mask(E, C, beta=args.beta, window=args.window)
...
# --- Strategy 12: SIFiD + Scaled Entailment ---
def agg_sifid_scaled_ent(E, Neu, C):
    """
    SIFiD document filtering followed by scaled entailment.
    Multiplicative gating after noise removal — double defense
    against both off-topic doc sentences and uncertain predictions.
    """
    mask = sifid_mask(E, C, beta=args.beta, window=args.window)
    E_f, Neu_f, C_f = E[mask], Neu[mask], C[mask]
    if E_f.shape[0] == 0:
...
def load_checkpoint(name):
    if os.path.exists(args.checkpoint):
        ...
        if data.get("dataset_name") == name:
            ...
            # If checkpoint is missing any current strategies, start fresh
            # (avoids score/label length mismatch after adding strategies)
